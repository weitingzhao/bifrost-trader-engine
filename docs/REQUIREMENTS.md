...
- **目标**：为**复盘与风控分析**提供**辅助行情数据**（如 **K 线**、历史 tick 或 OHLC 汇总），便于在复盘时结合成交记录查看当时行情与风险模型评估。
- **范围**：数据源可为 IB 历史数据接口、或既有 sink 中已有行情；形态可为独立脚本/模块按标的与周期拉取并写入库、或守护程序在连接 IB 时按配置写入历史 K 线。至少支持本策略涉及标的的**日线或所需周期**的 K 线（开高低收、成交量等）；供复盘页或统计脚本只读使用。
- **与分步计划**：阶段 3（数据获取）。
...
*最后更新：2026-02-27，新增 R-A2（账户执行交易可获取）、R-A3（复盘辅助行情可获取）、R-M7（复盘与风控分析页面），纳入阶段 3。*
