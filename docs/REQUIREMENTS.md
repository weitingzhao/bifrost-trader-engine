...
- **范围**：
  - **标的**：除当前持仓外，支持 **Wishlist**（自选/待操作标的，可含股票与期权，含当前持仓、未持仓与曾持仓）；Wishlist 落库持久化，服务重启不丢失。
  ...
  - **报价**：持仓与 Wishlist 标的的**当前报价**（bid/ask/last/mid）可获取；Wishlist 的报价在拉取后**写入 instrument_prices**（与持仓共用），供前端统一展示与后续使用。
...
*最后更新：2026-03-03，扩展 R-A3（复盘辅助行情）：股票+期权分表 K 线、Wishlist 落库、多周期、智能拉取、Wishlist 报价写入 instrument_prices。*
