...
### 2.14.9 Massive Stocks 基本面原始表（SEPA Data Ready Steps 4–9）

以下六表由 `db_refresh_schema` / `pg_ddl` 创建；写入路径为 Celery `job_massive_backfill` + `run_massive_job`，kind 分别为 `feed_stocks_income_statements`、`feed_stocks_balance_sheets`、`feed_stocks_cash_flows`、`feed_stocks_ratios`、`feed_stocks_short_interest`、`feed_stocks_short_volume`。REST 端点与缺口逻辑见 [`src/research/sepa/financials_data.py`](../src/research/sepa/financials_data.py) 与 Research API `GET/POST /research/screening/sepa/readiness/*-gaps` / `backfill-*`。

| 表名 | 主键 | Massive / Polygon REST（v1 财务三表） |
|------|------|----------------------------------------|
...
**SEPA Data Ready 推荐顺序（扩展后）**：Step 1–3 不变 → Step 4–9 六张基本面表回补 → Step 10 评估写入 `research_sepa_fundamentals_cache` → Step 11 `sepa_universe_readiness_daily` 物化 → Step 12 复核。
...
| 2026-05-05 SEPA Step 2 by-type 分布 | `fetch_sepa_readiness_summary` 增加 `stock_unified_snapshot_by_type`：`cache_stock_snapshot JOIN tickers GROUP BY instrument_type LEFT JOIN ticker_types`（`asset_class='stocks', locale='us'`）；返回 code / description / snapshot_row_count / universe_ticker_count，前端 SEPA Data Ready Step 2 卡内表格化展示。§2.14.6。 | 研究 / SEPA |
...
| 2026-05-11 `stock_readiness_daily` 新增 Technical Conditions Snapshot（4 列） | 新增 4 列用于按日存储 SEPA 技术条件评估结果：`technical_pass`（boolean DEFAULT false，全部 11 条通过）、`technical_pass_count`（integer DEFAULT 0，通过条数 0–11）、`technical_insufficient`（boolean DEFAULT false，<252 bars 数据不足）、`technical_eval`（jsonb NULL，完整评估结果含 conditions[] 与 metrics{}）。`technical_eval.conditions` 数组包含 11 条件（10 条 Phase-1 来自 `phase1_engine.evaluate_symbol_phase1`，第 11 条 `crs_ge_70` 来自 `crs_engine.compute_crs_scores` 的全宇宙百分位排名）。新增计算函数 `run_technical_local_backfill(status_config, symbols, *, min_crs=70.0, lookback_days=420)` 调用 `get_stock_day_series_for_sepa` + `get_stock_day_close_series_for_crs` 批量读取本地 `stock_day`，无外部 API；批量 UPSERT 写入。新增路由 `POST /research/data/readiness/backfill-technical`（默认 `only_missing=true`）后台线程调用。`compute_sepa_criteria_stats` 返回的 `technical` 字段新增 `tech_cached_count` / `tech_pass_count` / `tech_insufficient_count` 与 `conditions[]`（per-condition pass/fail 计数，使用 `jsonb_array_elements(technical_eval->'conditions')` 聚合）。`_SNAPSHOT_INSERT_SQL` 占位新增 4 列且 ON CONFLICT 保留已写入的 `technical_eval`，与 `fundamental_eval` 一致。 | 研究 / SEPA |
