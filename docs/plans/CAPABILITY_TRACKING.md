...
本文档按**木桶原理**列出达成**终极目标（自动交易）**所需的各方面能力，并标注当前进度，便于**分项跟踪**、发现短板。与 [PHASE_ASSESSMENT.md](PHASE_ASSESSMENT.md)「当前项目进展」、[PLAN_NEXT_STEPS.md](../PLAN_NEXT_STEPS.md) 阶段划分一致；**每次阶段评估或实现完成后可更新本表**。
...
| **21** | **交易归属与策略实例（按策略 PnL）** | 阶段 3 扩展 | ❌ 未实现 | strategy_instance 表；position/execution 归属 strategy_opportunity 与 strategy_instance；Performance/复盘按策略与实例聚合；步骤与验收见 [PLAN_NEXT_STEPS.md](../PLAN_NEXT_STEPS.md)「策略实例与交易归属」。实现后可按策略、按单笔开仓查看 PnL 与盈亏比，补齐 R-M7/R-H2 的按策略维度。 |
...
**更新约定**：实现或验收完成某能力维度后，将上表「当前进度」更新为 ✅ 并简要注明；发现新短板或新维度时可增行。详细阶段完成状态与里程碑见 [PHASE_ASSESSMENT.md](PHASE_ASSESSMENT.md)。
