...
| **21** | **交易归属与策略实例（按策略 PnL）** | 阶段 3 扩展 | ❌ 未实现 | strategy_instance 表；execution 归属 strategy_opportunity 与 strategy_instance（position 不存策略，从 executions 推导 strategy_links）；Performance/复盘按策略与实例聚合；**Strategy → Instances**（或等价入口）展示列表/详情与按实例盈亏。表结构见 [DATABASE.md](../DATABASE.md) §2.24.11。实现后可按策略、按单笔开仓查看 PnL 与盈亏比，补齐 R-M7/R-H2 的按策略维度。 |
...
| **23** | **Massive 期权研究数据（Option Discovery 主力源）** | R-A6，R-OD1，R-A3 | ⚠️ 部分实现 | 已实现：`massive` 配置与 REST 客户端、`job_massive_backfill` + Celery `massive` 队列、`GET /research/massive/status|option-snapshots|option-oi|option-trades`、`POST /research/massive/sync`、expirations `provider=auto|ib|massive`、`GET /bars` 支持 `asset=option`+`source`、Option Discovery UI（Massive badge、延迟说明、同步+轮询、Greeks/OI 列）。**Greeks/IV 已升级为完整能力**：四 tab（Chain Snapshot / Contract Snapshot / DB Verify / Unified）、`GET /research/massive/greeks-coverage` 覆盖率统计、job result 含 `rows_with_iv`/`rows_with_full_greeks`、质量摘要区（IV%、full greeks%、OI、stale rows）。**Contracts 已升级为完整能力**：四 tab（Contracts List / Contract Detail / DB Verify / Snapshot Link）、`MassiveClient.fetch_option_contracts_list`+`fetch_option_contract_detail`、Celery `kind=feed_option_contracts` 含 list/detail 模式及 summary 字段、`GET /research/massive/contracts-coverage` 覆盖率与映射一致性统计、前端质量摘要区（ticker mapping%、expirations、strikes、mismatches、stale）、contract→snapshot 端到端验证链路。**Market Ops 已覆盖为只读能力**：四 tab（Conditions / Exchanges / Market Holidays / Market Status）、`MassiveClient.fetch_market_conditions`+`fetch_market_exchanges`+`fetch_market_holidays`+`fetch_market_status`、`GET /research/massive/market-ops/*` 四个只读接口、Holiday tab 含 Massive vs 本地 `reference_us_holidays` 对照摘要；不落库、不改现有写入链路。**Technical Indicators 已覆盖为只读能力**：四 tab（SMA / EMA / RSI / MACD）、统一 `MassiveClient._fetch_indicator` + 各指标方法、`GET /research/massive/technical-indicators/{indicator}/{ticker}` 通用只读接口、支持 Option ticker（O: 前缀）与股票 ticker、每个 tab 含 Option applicability 适用说明；不落库、不改现有写入链路。**Massive WS ingest 运维**：`run_massive_ws.py` → Redis `bifrost:console:ws_massive_option` + Monitor `/api/massive-ws/logs*`；Settings → Market ingest；Ops `GET/POST /ops/market-ingest/*`；示例 unit `deploy/systemd/bifrost-massive-ws.service`。可选：Massive WS → 前端 SSE 聚合仍未做。详见 [ARCHITECTURE.md](../ARCHITECTURE.md) §2.10；schema 与迁移见 [DATABASE.md](../DATABASE.md)。 |
| **24** | **股票筛选（SEPA）** | R-A8（规划） | ⏳ 数据就绪 / 引擎未建 | 技术面数据已具备（`stock_day` OHLCV + SMA/grouped daily 计算基础），基本面接口已具备（Massive `vX/reference/financials` 经 fundamentals 路由提供 EPS/Revenue 读取）。当前缺口：CRS 排名计算、SEPA 筛选引擎模块、筛选 API 与前端结果页；可选补强：`stock_financials` 缓存落库与定时刷新。 |
...
- **环境与部署（R-DV4）**：调试拓扑下单 Engine 写唯一 Redis、Dev/Prod 多套 Market 可只读同一实例；需求见 [REQUIREMENTS.md](../REQUIREMENTS.md) §7.4，架构见 [ARCHITECTURE.md](../ARCHITECTURE.md) §2.8。
