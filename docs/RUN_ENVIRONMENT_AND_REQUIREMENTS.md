...
| **控制** | **R-C3** | 一键平敞口（安全兜底）：红时可 **一键平掉本策略管理的对冲敞口**；仅针对本守护程序负责的对冲仓位。依赖 R-A1（账户与持仓）及策略边界等。 | 待实现 | 阶段 3.6 或更后 | §5.1、§6 | 同上：R-C3 验收标准；该阶段 Test Case 待规划时补全 |
...
| **交易基础** | **R-A1** | 账户与持仓可获取：自动交易程序能从 IB 获取当前账户基本信息（账户 ID、Balance 等）与当前持仓，作为下一步自动交易对冲的基本能力。 | 待实现 | 阶段 3.0 | §4.4、§6 | [PLAN_NEXT_STEPS](PLAN_NEXT_STEPS.md)：R-A1 验收标准 + 阶段 3.0 Test Case 清单 |

**使用说明**：  
- **需求编号 ↔ 说明章节**：上表「说明章节」列指出该需求在本文档中的**细节描述位置**（如 R-M2 细节在 §4.1）；便于从分步计划中的需求编号反查到具体描述。  
  **快速对照**：R-M1、R-M3 → §4；R-M2 → §4.1；R-M4 → §4.2；R-M5 → §4.3；R-A1 → §4.4；R-C1、R-C3 → §5.1；R-C2 → §5.2；R-H1、R-H2 → §7；R-B1、R-B2 → §8；RE-6 → §3.2；RE-7 → §3.3；§6 为小结表。  
...
### 4.4 账户与持仓可获取（R-A1，自动交易对冲前置）

**需求编号：R-A1**

- **目标**：自动交易程序能够从 IB **获取当前账户基本信息**（如账户 ID、Balance、NetLiquidation 等）以及 **当前持仓**（股票、期权等）。这是进行自动交易对冲的**基本前置能力**：对冲逻辑依赖“当前账户与持仓”才能正确计算需平/需开的数量与方向。
- **范围**：
  - **账户信息**：至少包含账户标识、现金/权益类汇总（如 TotalCashValue、NetLiquidation、BuyingPower 等，以 IB API 实际提供为准）。
  - **当前持仓**：至少包含本策略可能涉及的标的（如标的股票、相关期权腿）的持仓数量与方向，可供守护程序内对冲逻辑与风控使用。
- **实现形态**：由守护程序在连接 IB 后通过 IB API 请求账户摘要与持仓，并可在 heartbeat 或状态 sink 中写入摘要（供监控展示可选）；核心验收为“守护程序内部能可靠获取并用于后续对冲决策”。
- **与分步计划**：本需求安排在 **阶段 3.0**（账户与持仓可获取），在阶段 2（监控/控制）之后、历史统计与回测之前完成，作为“下一步自动交易对冲”的基础。
...
| 账户与持仓可获取 | 自动交易程序能从 IB 获取当前账户基本信息（账户 ID、Balance 等）与当前持仓，作为自动交易对冲的基本能力。 | R-A1 |
