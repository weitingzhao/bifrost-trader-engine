...
- **同一台机器**：TWS（或 IB Gateway）与守护进程必须同机；手动交易与该机共享账户（不同 client_id）。
- **可选同机或局域网**：独立监控/控制应用可跑在同一台机器或局域网内另一台设备，只读 sink 输出、写控制文件（或调守护进程暴露的控制 API）。
...
```
                    ...
                    └───────────────┬───────────────┘
                                    ...
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  同一台机器 (Mac 或 Linux 服务器)                                          │
│  ┌─────────────┐   ┌─────────────────────────────────────────────────┐  │
│  │ TWS/Gateway │   │ 守护进程  →  StatusSink → SQLite(当前+历史)       │  │
│  │             │◄──│             ← 控制文件轮询                        │  │
│  └─────────────┘   └─────────────────────────────────────────────────┘  │
│  手动交易：TWS UI 或另一 client_id 的客户端（本机或远程桌面）               │
└─────────────────────────────────────────────────────────────────────────┘
```
