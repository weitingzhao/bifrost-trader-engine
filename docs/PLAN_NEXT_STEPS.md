...
| 步骤 | 内容 | 可交付物 |
|------|------|----------|
| **SI.1** | 数据库：表 strategy_instance；account_executions 新增 strategy_opportunity_id、strategy_instance_id（**account_positions 不存策略归属**，策略信息从 executions 推导）；pg_ddl/迁移。 | 表存在且 FK 正确；旧数据列为 NULL；account_positions 无 strategy 列。 |
| **SI.2** | Reader/API：GET /status 的 positions 通过子查询返回 `strategy_links[]`（从 account_executions 推导 DISTINCT 策略列表）；GET /executions 支持按 strategy_opportunity_id、strategy_instance_id 筛选并返回两字段；`PATCH /executions/strategy-attribution` 批量打标（替代 PUT /positions/strategy）；GET /strategies/instances（列表/详情，按 account + opportunity 可选）；POST 创建 strategy_instance。 | API 文档与验收用例通过。 |
| **SI.3** | 写入路径：① 前端 Add Trade / 从 Opportunity 下单：可传 strategy_opportunity_id、strategy_instance_id（新建实例时先 POST instance 再写 execution）；② IB 拉取 account_executions：默认 NULL，可选后处理或人工 tagging via PATCH /executions/strategy-attribution；③ Daemon 对冲：若支持「当前 opportunity」，写入 execution 时带 strategy_opportunity_id（及可选 strategy_instance_id）。 | 至少一种路径能写入归属；手动补录可带 opportunity/instance。 |
...
**验收标准**（精简）：① 表与 FK 存在且可读写；account_positions 无 strategy 列。② GET /status 的 positions 含 `strategy_links[]`（从 executions 推导）；GET /executions 含归属字段并可筛选。③ 至少一种写入路径（如前端 Add Trade + 选 Opportunity/创建 Instance 或 PATCH /executions/strategy-attribution）能落库。④ GET /performance 或等价接口支持按 strategy_opportunity_id 聚合并展示。⑤ **策略实例独立页面**：从导航可进入策略实例列表；列表支持按账户、机会筛选并展示实例元数据；点击实例进入详情，展示策略信息、成交与绩效（按 instance 筛选）；详情页预留风险/回测/资金占用扩展位。
