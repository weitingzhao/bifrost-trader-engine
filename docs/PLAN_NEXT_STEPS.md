...
| **阶段 3**（数据获取） | **R-A1**、**R-A4**、**R-A2**、**R-A3**、**R-M6**、**R-M7**、**R-H2** | 账户、持仓、市值、账户执行交易与辅助行情、复盘页、历史统计等数据的获取（供策略与监控使用）；双 IB 账户与统一 Portfolio |
...
| **R-M7** | 复盘与风控分析页面：提供独立于实时监控的复盘与风控分析页面，用于查看账户执行交易、辅助行情（如 K 线）及风险模型评估。 | **阶段 3** | ① 监控应用提供**独立页面或路由**（如「复盘」/「风控」），与当前交易监控页**分离**，通过导航切换。② 复盘页可展示**账户执行交易记录**（R-A2 数据）、**辅助行情**（R-A3，如 K 线或 OHLC）及基于历史数据的风险/统计视图。③ 不要求与 R-M5 同屏；数据由 GET /executions（或等效）及 K 线/行情接口提供。④ Performance 页计算与展示的细化验收见 [performance-execution-plan.md](plans/performance-execution-plan.md)。 |
...
| **R-A4** | 双 IB 账户与统一 Portfolio：支持两账户；主账户自动+行情，第二账户仅手动；统一 Portfolio 展示与管理。 | **阶段 3** | ① 配置可指定主账户（如 `primary_account_id`），未配置时行为与现有一致（取 managed accounts 首个）。② 守护进程仅对主账户执行对冲与行情相关逻辑；R-C3 一键平敞口仅针对主账户。③ 监控/复盘页可展示两账户的持仓、执行、Performance 按账户拆分；GET /status 的 accounts、GET /executions、GET /performance 支持多账户。 |
...
| **R-H2** | 历史统计：基于历史数据做胜率、盈亏分布、按日/周/月汇总、对冲次数与滑点等。 | **阶段 3** | ① 存在**独立脚本或模块**（如 `scripts/stats_from_history.py` 或 `src/stats/`），**只读**阶段 1 sink 写入的历史表。② 数据来源与守护程序写出一致，**不跑** FSM/Guard/StateClassifier。③ 输出至少包含：**按日/周对冲次数**、**盈亏分布或汇总**；可选滑点、按月汇总等。④ 统计可**离线运行**，不依赖守护进程在线。⑤ Performance 页计算逻辑与按日/周/月汇总、胜率、盈亏分布的细化验收见 [performance-execution-plan.md](plans/performance-execution-plan.md)。 |
...
| **3.8** | **Performance 计算与展示（R-M7/R-H2 细化）**：按 [PERFORMANCE_PAGE_DESIGN.md](PERFORMANCE_PAGE_DESIGN.md) §7 与 [performance-execution-plan.md](plans/performance-execution-plan.md) 分步实现；GET /performance 当前为 stub，需完成 Phase 0（资本与资金流口径）后依次实现 Phase 1–8；验收以 performance-execution-plan 的验收清单与各 Phase 校验条件为准。 | GET /performance 返回 Realized/Unrealized/Transaction 与 %；Performance 页分块展示 | R-M7、R-H2 |
...
**阶段通过条件**：R-A1、R-A2、R-A3、R-M6、R-M7、R-H2 验收条全部通过，即上述 Test Case 全部通过。通过后可进入阶段 4。
