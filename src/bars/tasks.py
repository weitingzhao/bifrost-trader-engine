...
_project_root = _here.parent.parent  # src/bars/ -> src/ -> project root
...
async def _run_backfill_in_loop(
    ...
    """
    global _worker_last_bars_job_finished_ts, _worker_last_bars_job_interval_sec
    from src.monitor.reader import StatusReader, get_job_bars_backfill, update_job_bars_backfill_result
    from src.bars.backfill import run_one_backfill
...
def backfill_bars(
    ...
    try:
        from src.app.config import read_config
        from src.monitor.reader import update_job_bars_backfill_result
