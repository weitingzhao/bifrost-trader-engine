"""Merge ``ib_gateway:`` from YAML with defaults."""
...
def effective_ib_gateway_settings(config: Dict[str, Any]) -> Dict[str, Any]:
    """Return resolved IB Gateway settings (stream, group, TTLs, keys).

    ``enabled`` defaults to True when Redis URL is available and ``ib_gateway.enabled``
    is not explicitly False.
    """
    raw = config.get("ib_gateway") or {}
    ...
    return {
        ...
        "redis_url": rurl,
        "stream": (raw.get("stream") or "bifrost:ib:cmd").strip() or "bifrost:ib:cmd",
        "consumer_group": (raw.get("consumer_group") or "ib-gateway").strip() or "ib-gateway",
        "result_prefix": (raw.get("result_prefix") or "bifrost:ib:result:").strip()
        or "bifrost:ib:result:",
        "health_key": (raw.get("health_key") or "bifrost:ib:gateway:health").strip()
        or "bifrost:ib:gateway:health",
