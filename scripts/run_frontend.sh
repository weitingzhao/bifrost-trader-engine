...
#
# 使用方式:
#   ./scripts/run_frontend.sh dev    # 开发模式，监听 0.0.0.0，端口从 config/config.yaml frontend.port 读取，启动前会 Kill 占用端口的进程
...
case "$cmd" in
  dev)
    ...
    echo "前端开发端口: $FRONTEND_PORT（来自 config/config.yaml frontend.port），监听 0.0.0.0"
  ...
  *)
    ...
    echo ""
    echo "  dev     - 启动开发服务器（监听 0.0.0.0，端口见 config/config.yaml frontend.port，默认 5173）"
