#!/usr/bin/env python3
"""Standalone Portfolio API server — portfolio models, holdings, allocation.

Console logs are mirrored to Redis (bifrost:console:{dev|prod}:api_portfolio) for Settings → API → Account,
same pattern as Ops/Docs. Default config: config/config.dev.yaml. Use --prod or BIFROST_ENV=prod for prod."""
...
_PORTFOLIO_LOG_STREAM_MAXLEN = 50
...
def setup_logging(*, portfolio_log_stream_key: str, redis_url: str | None = None) -> None:
    ...
    redis_handler = RedisStreamLogHandler(
        resolved_redis,
        portfolio_log_stream_key,
        maxlen=_PORTFOLIO_LOG_STREAM_MAXLEN,
    )
...
def main() -> None:
    ...
    setup_logging(
        portfolio_log_stream_key=portfolio_api_console_stream_key(profile),
        redis_url=redis_url,
    )
    print(f"bifrost portfolio server: YAML loaded: {resolved_config_path}", file=sys.stderr)
    port = int(config["server"]["portfolio_port"])
    if not _free_port(port):
        print(f"Could not free port {port}.", file=sys.stderr)
        sys.exit(1)
    from backend.portfolio.app import run_portfolio_server
    run_portfolio_server(config, resolved_config_path=resolved_config_path)
