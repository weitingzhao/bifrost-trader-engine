#!/usr/bin/env python3
"""Fetch a single IB Flex Trades report using Secondary token and a given Query ID.

Use this to verify that the Flex API returns data for Query ID 1428413 (or another ID)
...
If token is not provided and DB has no token, set IB_FLEX_SECONDARY_TOKEN in the environment.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Pull Secondary IB Flex Trades report by Query ID using Secondary token."
    )
    parser.add_argument(
        "query_id",
        nargs="?",
        default="1428413",
        help="Flex Query ID (default: 1428413)",
    )
    parser.add_argument(
        "--token",
        default=os.environ.get("IB_FLEX_SECONDARY_TOKEN", "").strip(),
        help="Flex token (default: from IB_FLEX_SECONDARY_TOKEN or DB settings)",
    )
    parser.add_argument(
        "--config",
    ...
    query_id = (args.query_id or "1428413").strip()
            ...
            if config.get("postgres") or os.environ.get("PGHOST"):
                try:
                    ...
                    with conn.cursor(cursor_factory=RealDictCursor) as cur:
                        cur.execute(
                            "SELECT ib_flex_secondary_token FROM settings WHERE id = 1"
                        )
                        row = cur.fetchone()
                    conn.close()
                    if row and row.get("ib_flex_secondary_token"):
                        token = (row["ib_flex_secondary_token"] or "").strip()
                except Exception as e:
                    print(f"DB read failed: {e}", file=sys.stderr)
            else:
                print(
                    "No postgres in config and no PGHOST. Use --token or set IB_FLEX_SECONDARY_TOKEN.",
                    file=sys.stderr,
                )
                return 1
        else:
            print(
                "No config file. Use --token or set IB_FLEX_SECONDARY_TOKEN.",
                file=sys.stderr,
            )
            return 1
    ...
    if not token:
        print(
            "Token is empty. Configure in Settings → IB Connection → Flex (Secondary token) or pass --token.",
            file=sys.stderr,
        )
        return 1
    ...
    if args.last_365:
        use_period_5 = True
        print(
            "Using Flex period p=5 (Last 365 Calendar Days), no fd/td.",
            file=sys.stderr,
        )
    elif args.mimic_web:
        end = date.today()
        ...
        print(
            "Mimic web: fd/td = today-365 .. today (%s .. %s)"
            % (from_date, to_date),
            file=sys.stderr,
        )
        ...
        print(
            "Date range (explicit): %s .. %s" % (from_date, to_date),
            file=sys.stderr,
        )
    else:
        print(
            "No date range (using Flex query default period).",
            file=sys.stderr,
        )

    print("Query ID: %s" % query_id, file=sys.stderr)
    print(
        "Token: %s..." % (token[:8] if len(token) > 8 else "***"),
        file=sys.stderr,
    )
    if use_period_5:
        print(
            "Calling Flex SendRequest (p=5, Last 365 Calendar Days)...",
            file=sys.stderr,
            flush=True,
        )
    elif from_date and to_date:
        print(
            "Calling Flex SendRequest (fd=%s, td=%s)..."
            % (from_date, to_date),
            file=sys.stderr,
            flush=True,
        )
        ...
        print(
            "Calling Flex SendRequest (no date range, use query default)...",
            file=sys.stderr,
            flush=True,
        )
    ...
    try:
        ...
        print(
            f"SendRequest OK. ReferenceCode: {ref}",
            file=sys.stderr,
            flush=True,
        )
        ...
        print(
            f"GetStatement OK. Response length: {len(body)} chars",
            file=sys.stderr,
            flush=True,
        )
        ...
        if rows:
            ...
            for i, r in enumerate(rows[:3]):
                print(
                    f"  [{i+1}] {r.get('symbol')} {r.get('side')} {r.get('quantity')} @ {r.get('price')}  time={r.get('time')} account={r.get('account_id')}",
                    file=sys.stderr,
                )
        else:
            print(
                "No trades in report (report may be Cash Transactions or empty date range).",
                file=sys.stderr,
            )
