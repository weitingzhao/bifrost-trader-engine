#!/usr/bin/env python3
"""Standalone Market API server — market data, quotes, watchlists.

Console logs are mirrored to Redis (bifrost:console:{dev|prod}:api_market) for Settings → API → Research,
...
def setup_logging(*, market_log_stream_key: str, redis_url: str | None = None) -> None:
    ...
    redis_handler = RedisStreamLogHandler(
        resolved_redis,
        market_log_stream_key,
        maxlen=_MARKET_LOG_STREAM_MAXLEN,
    )
...
def main() -> None:
    ...
    setup_logging(
        market_log_stream_key=market_api_console_stream_key(profile),
        redis_url=redis_url,
    )
    print(f"bifrost market server: YAML loaded: {resolved_config_path}", file=sys.stderr)
    port = int(config["server"]["market_port"])
    if not _free_port(port):
        print(f"Could not free port {port}.", file=sys.stderr)
        sys.exit(1)
    from backend.market.app import run_market_server

    run_market_server(config, resolved_config_path=resolved_config_path)
