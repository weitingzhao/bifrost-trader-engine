#!/usr/bin/env python3
"""Standalone Strategy API server — strategy definitions, signals, scoring.

Console logs are mirrored to Redis (bifrost:console:{dev|prod}:api_strategy) for Settings → API → Research,
...
_STRATEGY_LOG_STREAM_MAXLEN = 50
...
def setup_logging(*, strategy_log_stream_key: str, redis_url: str | None = None) -> None:
    ...
    redis_handler = RedisStreamLogHandler(
        resolved_redis,
        strategy_log_stream_key,
        maxlen=_STRATEGY_LOG_STREAM_MAXLEN,
    )
...
def main() -> None:
    ...
    setup_logging(
        strategy_log_stream_key=strategy_api_console_stream_key(profile),
        redis_url=redis_url,
    )
    ...
    if not _free_port(port):
        ...
        sys.exit(1)
    from backend.strategy.app import run_strategy_server

    run_strategy_server(config, resolved_config_path=resolved_config_path)
