#!/usr/bin/env python3
"""Standalone Massive API server — /research/massive/* endpoints + SSE stream.

On startup, reads server.massive_port from config and frees the port if
already in use, then starts the Massive FastAPI via backend.massive.app.
...
from servers.routers.deps import MASSIVE_LOG_STREAM_KEY
...
def setup_logging() -> None:
    ...
    redis_handler = RedisStreamLogHandler(
        ...
        maxlen=_MASSIVE_LOG_STREAM_MAXLEN,
    )
...
def main() -> None:
    ...
    print(f"bifrost massive server: YAML loaded (env file): {resolved_config_path}", file=sys.stderr)
    _rp = Path(resolved_config_path)
    if _rp.name in ("config.dev.yaml", "config.prod.yaml"):
        _base = _rp.parent / "config.yaml"
        if _base.is_file():
            print(
                f"bifrost massive server: merged config base {_base} + env {_rp.name} (env wins on conflicts).",
                file=sys.stderr,
            )
        else:
            print(
                f"bifrost massive server: no {_base.name} beside env file; using {_rp.name} only.",
                file=sys.stderr,
            )

    port = _massive_port_from_config(config)
