...
import type { IbAccountSnapshot, StatusResponse, Operation, RiskSummaryResponse, Execution, Bar, OptExecutionGroup } from './types'
import {
  fetchStatus,
  fetchOperations,
  fetchRiskSummary,
...
function formatContractLabel(contract_key: string): string {
  const { symbol, rightLabel } = getContractLabelParts(contract_key)
  return [symbol, rightLabel].filter(Boolean).join(' ')
}
...
export default function App() {
  ...
  /** 拉取执行记录的时间范围：1=当天, 3=最近3天, 7=最近7天（需 TWS Trade Log 勾选） */
  const [replayFetchDays, setReplayFetchDays] = useState<1 | 3 | 7>(1)
  /** 拉取 K 线时使用的标的：用户输入或从持仓/交易记录汇总的候选 */
  const [replayBarSymbol, setReplayBarSymbol] = useState('')

  /** 复盘页可选标的 = 当前状态标的 + 交易记录中的标的 + 账户持仓中的标的（去重） */
  const replayBarCandidateSymbols = useMemo(() => {
    ...
    const fromAccounts = (status?.accounts || []).flatMap(
      acc => (acc.positions || []).map(p => p.symbol).filter((s): s is string => Boolean(s?.trim()))
    )
    return [...new Set([...fromStatus, ...fromExec, ...fromAccounts])]
  }, [riskSummary?.symbol, executions, status?.accounts])

  /** 期权执行记录按 contract_key + strike 分组，用于兑现/未兑现展示（仅 OPT） */
  const optExecutionGroups = useMemo((): OptExecutionGroup[] => {
    ...
    for (const [, trades] of groups) {
      ...
      const realized_pnl = sell_value - buy_value
  ...
  useEffect(() => {
    if (activeTab === 'replay') loadReplayData()
  }, [activeTab, loadReplayData])
  ...
  const tabList: { id: TabId; label: string; lamp?: 'green' | 'yellow' | 'red' | 'none' }[] = [
    ...
    { id: 'ib', label: 'IB 账户' },
    { id: 'replay', label: '复盘与风控' },
  ]

  return (
    <div className="app">
      <h1>Bifrost 自动交易监控</h1>
      <p className="app-subtitle">通过下方主菜单切换：监控与控制 · IB 账户 · 复盘与风控</p>
      ...
      <nav className="app-tabs" aria-label="主菜单：在监控、IB 账户、复盘与风控等页面间切换">
      ...
      <div className="card process-section replay-page">
        <h2>复盘与风控分析</h2>
        <p className="section-desc">事后复盘、交易记录与风险评估；与实时监控分离，供独立分析。</p>
        ...
        <section className="replay-section" aria-labelledby="trade-records-head">
          ...
          <p className="section-hint">来自 GET /operations；账户级执行记录（R-A2）实现后将在下方「账户执行」中展示。</p>
          <table className="table-operations">
            <thead>
              <tr>
                <th>时间</th>
          ...
          {executions.length === 0 ? (
            ...
            <>
              {/* 期权：按 contract_key + strike 分组，兑现/未兑现 */}
              {optExecutionGroups.length > 0 && (
                <>
                  <p className="section-hint">期权按 contract_key 与 strike 分组；买均价/卖均价为加权平均（$/股）；盈利=(卖均价×卖手数−买均价×买手数)×100−手续费，按状态着色（已兑现绿、未兑现黄）。</p>
                      ...
                      {optExecutionGroups.map((g, idx) => {
                        ...
                        return (
                          <tr key={`${g.contract_key}-${g.strike}-${idx}`}>
                            <td className="replay-opt-contract">{(() => { const p = getContractLabelParts(g.contract_key); return p.symbol ? <><strong>{p.symbol}</strong> {p.rightLabel}</> : formatContractLabel(g.contract_key); })()}</td>
                    ...
                    <tbody>
                      {optExecutionGroups.flatMap((g, gi) =>
                        g.trades.map((ex, ti) => {
                          ...
                          const sideLabel = s === 'BUY' || s === 'BOT' || s === 'B' ? '买' : s === 'SELL' || s === 'SLD' || s === 'S' ? '卖' : (ex.side ?? '—')
                          return (
                            <tr key={`${gi}-${ti}-${ex.time ?? ti}`}>
                              <td>{(() => { const p = getContractLabelParts(g.contract_key); return p.symbol ? <><strong>{p.symbol}</strong> {p.rightLabel} {fmtUsd(g.strike)}</> : <>{formatContractLabel(g.contract_key)} {fmtUsd(g.strike)}</>; })()}</td>
        ...
        <section className="replay-section" aria-labelledby="bars-head">
          ...
          <p className="section-hint">按标的拉取日线，供复盘时结合成交时间查看当时行情。下方可输入标的代码，或从「当前持仓 + 交易记录」汇总的候选中选择。</p>
