...
export type ResearchLogSource = 'research' | 'strategy' | 'market'

export interface UseResearchUnifiedLogConsoleOptions {
...
export function useResearchUnifiedLogConsole({
  initialHeightPx = 280,
  initialMaxLines = 50,
  enabled = true,
}: UseResearchUnifiedLogConsoleOptions): UnifiedAggregatedLogConsoleController {
  ...
  useEffect(() => {
    ...
    const appendLine = (source: ResearchLogSource, line: string) => {
      if (cancelled) return
      idRef.current += 1
    ...
    void Promise.allSettled([fetchResearchLogs(limit), fetchStrategyLogs(limit), fetchMarketLogs(limit)]).then(
      (results) => {
        if (cancelled) return
        ...
        const researchLines = getLines(0, 'Research')
        const strategyLines = getLines(1, 'Strategy')
        const marketLines = getLines(2, 'Market')

        const merged = mergeInitial(
          [
            { source: 'research', lines: researchLines },
            { source: 'strategy', lines: strategyLines },
            { source: 'market', lines: marketLines },
          ],
          limit,
        )
        ...
        streamUnsubsRef.current.push(subscribeResearchLogs((line) => appendLine('research', line), onStreamError))
        streamUnsubsRef.current.push(subscribeStrategyLogs((line) => appendLine('strategy', line), onStreamError))
        streamUnsubsRef.current.push(subscribeMarketLogs((line) => appendLine('market', line), onStreamError))
      },
    )
  ...
  const clearAllStreams = useCallback(async () => {
    setClearError(null)
    const settled = await Promise.allSettled([clearResearchLogs(), clearStrategyLogs(), clearMarketLogs()])
    const msgs: string[] = []
    const labels = ['Research', 'Strategy', 'Market'] as const
    settled.forEach((r, i) => {
      if (r.status === 'rejected') {
