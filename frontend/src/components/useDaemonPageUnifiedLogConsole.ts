...
export type DaemonPageLogSource = 'strategy_trading' | 'account_sync'

export interface UseDaemonPageUnifiedLogConsoleOptions {
...
/**
 * Merged live logs for Daemon page: Strategy Trading Daemon + Account Sync Daemon
 * (same Sources bubble UX as Socket → Logs on Market ingest Ops).
 */
export function useDaemonPageUnifiedLogConsole({
  initialHeightPx = 280,
  initialMaxLines = 500,
  enabled = true,
}: UseDaemonPageUnifiedLogConsoleOptions): UnifiedAggregatedLogConsoleController {
  ...
  useEffect(() => {
    ...
    const appendLine = (source: DaemonPageLogSource, line: string) => {
      if (cancelled) return
      idRef.current += 1
    ...
    void Promise.allSettled([fetchDaemonLogs(limit), fetchAccountSyncDaemonLogs(limit)]).then((results) => {
      if (cancelled) return
      ...
      const daemonLines = getLines(0, 'Strategy Trading Daemon')
      const accountSyncLines = getLines(1, 'Account Sync Daemon')

      const merged = mergeInitial(
        [
          { source: 'strategy_trading', lines: daemonLines },
          { source: 'account_sync', lines: accountSyncLines },
        ],
        limit,
      )
      ...
      streamUnsubsRef.current.push(
        subscribeDaemonLogs((line) => appendLine('strategy_trading', line), onStreamError),
      )
      streamUnsubsRef.current.push(
        subscribeAccountSyncDaemonLogs((line) => appendLine('account_sync', line), onStreamError),
      )
    })
  ...
  const clearAllStreams = useCallback(async () => {
    setClearError(null)
    const settled = await Promise.allSettled([clearDaemonLogs(), clearAccountSyncDaemonLogs()])
    const msgs: string[] = []
    const labels = ['Strategy Trading Daemon', 'Account Sync Daemon'] as const
    settled.forEach((r, i) => {
      if (r.status === 'rejected') {
...
export const DAEMON_PAGE_LOG_SOURCE_DEFINITIONS = [
  { source: 'strategy_trading', label: 'Strategy Trading' },
  { source: 'account_sync', label: 'Account Sync' },
] as const
