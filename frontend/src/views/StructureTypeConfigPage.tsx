              ...
              {DIM_TYPES.map((dt) => (
                <label key={dt} className="otc-filter-row">
                  ...
                  <AppSelect
                    ...
                    options={[{ value: '', label: 'All' }, ...(dimsByType[dt] || []).map((d) => ({ value: d.code, label: d.display_label }))]}
                  />
                </label>
              ))}
              {activeDimFilterCount > 0 && (
                  ...
                  {DIM_TYPES.map((dt) => (
                    <label key={dt} className="otc-dim-cell">
                      ...
                      <AppSelect
                        ...
                        placeholder="—"
                        options={(dimsByType[dt] || []).map((d) => ({ value: d.code, label: d.display_label }))}
                      />
                    </label>
                  ))}
                </div>
                          ...
                          <tr key={i}>
                            ...
                            <td>
                              <AppSelect
                                ...
                                onChange={(v) => {
                                  const legs = [...(detail.legs || [])]
                                  legs[i] = { ...legs[i], direction: v || null }
                                  setDetail({ ...detail, legs })
                                }}
                                options={legDirOpts}
                              />
                            ...
                            <td>
                              <AppSelect
                                ...
                                onChange={(v) => {
                                  const legs = [...(detail.legs || [])]
                                  legs[i] = { ...legs[i], option_right: v === '' ? null : v }
                                  setDetail({ ...detail, legs })
                                }}
                                options={legOrOpts}
                              />
          ...
          <div className="otc-dialog otc-dialog--wide" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="otc-dims-add-row">
              <AppSelect
                className="otc-select"
                value={newDimType}
                onChange={setNewDimType}
                options={DIM_TYPES.map((dt) => ({ value: dt, label: DIM_LABELS[dt] }))}
              />
                  ...
                  <td>
                    {valueOptsByKey[p.meta_key]?.length ? (
                      <AppSelect
                        ...
                        onChange={(v) => {
                          const mp = [...(detail.meta_params || [])]
                          mp[i] = { ...mp[i], default_value_text: v }
                          setDetail({ ...detail, meta_params: mp })
                        }}
                        placeholder="—"
                        options={(valueOptsByKey[p.meta_key] || []).map((o) => ({ value: o.value, label: o.label }))}
                      />
                    ) : (
                      <input
                        className="otc-input otc-input--compact"
