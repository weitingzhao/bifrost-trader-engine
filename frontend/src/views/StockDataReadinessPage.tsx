...
import { postMassiveSync, fetchPutCallRatioHistory } from '../api'
import {
  MassiveRefJobSessionProvider,
  useMassiveRefJobSession,
} from './massive/MassiveRefJobSessionContext'
import {
  fmt, fmtPct, fmtRelativeTime, copyTextFallback, sourceTag, splitObject,
} from './stockReadiness/stockReadinessUtils'
import { SnapshotByTypeBreakdown } from './stockReadiness/SnapshotBreakdownSection'
import { SepaScreeningChecklist } from './stockReadiness/SepaScreeningChecklist'
import { GapsDrawer } from './stockReadiness/GapsDrawer'
import {
  FinancialGapsDrawer,
  finGapOk,
  finDrawerTitleForKind,
  finDrawerColumnPresetForKind,
  finBackfillJobKindForDrawer,
  type FinDrawerKind,
} from './stockReadiness/FinancialGapsDrawer'
import { COND_DETAIL, FUND_SQL_AGGREGATION, FUND_SQL_SYMBOL, TECH_COND_LABELS } from './stockReadiness/readinessConstants'
...
/** Roll up child statuses into a single stage-level status.
 *
 *   - Any `loading` / `error` / `warn` child propagates upward (worst wins).
 *   - When every child is either `ok` or `void` (Source N/A, acknowledged),
 *     the stage is considered fully done and we report `ok` (green).
 *     Rationale: acked-void counts as "done" in per-step `done` logic, so
 *     keeping the stage at `void` (gray) while every step badge below it is
 *     green would mismatch the rest of the UI.
 *   - Mixed ok/void with other unknown children → `warn`.
 *   - Otherwise → `unknown` (nothing has been checked yet).
 */
function foldStageStatus(statuses: CheckStatus[]): CheckStatus {
  ...
  if (statuses.every((x) => x === 'ok' || x === 'void')) return 'ok'
...
function StockDataReadinessPageInner({
  ...
  // Option EOD Reports (EOD Pipeline: OI + Max Pain + Put/Call Ratio)
  const [eodBusy, setEodBusy] = useState(false)
  const [eodMsg, setEodMsg] = useState<string | null>(null)
  const [eodOk, setEodOk] = useState<boolean | null>(null)
  const [pcrBusy, setPcrBusy] = useState(false)
  const [pcrMsg, setPcrMsg] = useState<string | null>(null)
  const [pcrOk, setPcrOk] = useState<boolean | null>(null)
  const [pcrLastDate, setPcrLastDate] = useState<string | null>(null)
  ...
  // Load last PCR date on mount
  useEffect(() => {
    fetchPutCallRatioHistory({ symbol: 'NVDA', lookbackDays: 5 }).then(res => {
      if (res.ok && res.series.length > 0) {
        setPcrLastDate(res.series[res.series.length - 1].trade_date)
      }
    }).catch(() => {})
  }, [])

  const runEodPipeline = useCallback(async () => {
    setEodBusy(true)
    setEodMsg(null)
    setEodOk(null)
    try {
      const res = await postMassiveSync('eod_pipeline', {})
      if (res.ok) {
        setEodMsg(`Enqueued job ${res.job_id ?? ''}. OI → Max Pain → PCR will run when worker picks it up.`)
        setEodOk(true)
      } else {
        setEodMsg(res.error ?? 'Failed to enqueue')
        setEodOk(false)
      }
    } catch (e) {
      setEodMsg(e instanceof Error ? e.message : 'Network error')
      setEodOk(false)
    } finally {
      setEodBusy(false)
    }
  ...
  const runPcrOnly = useCallback(async () => {
    setPcrBusy(true)
    setPcrMsg(null)
    setPcrOk(null)
    try {
      const res = await postMassiveSync('report_option_put_call_ratio', {})
      if (res.ok) {
        setPcrMsg(`Enqueued job ${res.job_id ?? ''}. PCR report will compute when worker picks it up.`)
        setPcrOk(true)
      } else {
        setPcrMsg(res.error ?? 'Failed to enqueue')
        setPcrOk(false)
      }
    } catch (e) {
      setPcrMsg(e instanceof Error ? e.message : 'Network error')
      setPcrOk(false)
    } finally {
      setPcrBusy(false)
    }
  ...
  const runEvaluateAndPublish = useCallback(async () => {
    ...
    try {
      ...
      await loadSummary()
      void loadCriteriaStats()
      // Mark step 10 as checked so Check Results shows updated status strip.
      // Stage collapse state is left untouched — user explicitly opens stages.
      setCheckedSteps(prev => { const n = new Set(prev); n.add(10); return n })
    } catch (e) {
  ...
  }, [loadSummary, loadCriteriaStats])

  // Combined: Check Data Coverage → auto-run Evaluate & Refresh Snapshot
  // Note: Check Results stays in its current collapsed/expanded state — user can
  // open stages on demand via the stage headers or Expand all / Collapse all.
  const handleCheckAndEvaluate = useCallback(async () => {
    const res = await loadSummary()
    void loadQueues()
    if (res !== null) {
      setCheckedSteps(new Set<number>(ALL_SEPA_RUNBOOK_STEP_IDS))
      if ((res.universe_count ?? 0) > 0) {
        await runEvaluateAndPublish()
      }
    }
  }, [loadSummary, loadQueues, runEvaluateAndPublish])
      ...
      <div className="sdp-actions-card">
        <div className="sdp-runbook-actions-head">
          ...
          <button
            ...
            onClick={() => void handleCheckAndEvaluate()}
            disabled={summaryLoading || evalPublishBusy}
            title="Check data coverage for all steps, then evaluate & refresh snapshot"
          >
            {summaryLoading
              ? 'Checking…'
              : evalPublishBusy
              ? (evalPublishPhase === 'backfill' ? 'Evaluating…' : 'Refreshing snapshot…')
              : 'Check & Evaluate'}
          </button>
        </div>
                ...
                {stage.steps.map((s) => (
                  <button
                    ...
                    className={[
                      ...
                      checkedSteps.has(s.id) ? `sdp-runbook-tab--${s.status}` : '',
                    ].filter(Boolean).join(' ')}
        ...
        {/* ── Option EOD Reports ──────────────────────────────────────── */}
        <div className="sdp-runbook-stage sdp-option-eod-stage" style={{ marginTop: 'var(--space-4)' }}>
          <div className="sdp-runbook-stage-head sdp-runbook-stage-head--void">
            <div className="sdp-runbook-stage-head-top">
              <span className="sdp-runbook-stage-kicker">Option EOD</span>
              <span className="sdp-runbook-stage-title">Option EOD Reports</span>
              <span className="sdp-runbook-stage-subtitle">
                Runs watchlist EOD chain snapshot → daily OI → Max Pain → Put/Call Ratio.
                Schedule: after US market close.
              </span>
            </div>
          </div>
          <div className="sdp-runbook-stage-body" style={{ padding: 'var(--space-3) var(--space-4)' }}>
            {/* Status row */}
            <div style={{ display: 'flex', gap: 'var(--space-3)', flexWrap: 'wrap', marginBottom: 'var(--space-3)', fontSize: 'var(--text-caption)', color: 'var(--color-text-dim)' }}>
              <span>
                PCR last date:&nbsp;
                <strong style={{ color: pcrLastDate ? 'var(--color-text-main)' : 'var(--color-text-dim)' }}>
                  {pcrLastDate ?? '—'}
                </strong>
              </span>
              <span style={{ color: 'var(--color-text-dim)' }}>
                View results: Research → Option Discovery → select symbol → section 2 "Put/Call Ratio"
              </span>
            </div>

            {/* Action buttons */}
            <div className="sdp-step-actions" style={{ gap: 'var(--space-2)' }}>
              <button
                type="button"
                className="sdp-btn-primary"
                onClick={() => void runEodPipeline()}
                disabled={eodBusy}
                title="Enqueue full EOD pipeline: EOD OI snapshot + Max Pain + Put/Call Ratio for all watchlist symbols"
              >
                {eodBusy ? 'Enqueueing…' : 'Run EOD Pipeline'}
              </button>
              <button
                type="button"
                className="sdp-btn-secondary"
                onClick={() => void runPcrOnly()}
                disabled={pcrBusy}
                title="Enqueue Put/Call Ratio report only (requires EOD OI data from today already in DB)"
              >
                {pcrBusy ? 'Enqueueing…' : 'PCR Only'}
              </button>
            </div>

            {/* Feedback */}
            {eodMsg != null && (
              <div className={`sdp-feedback sdp-msg--${eodOk ? 'ok' : 'err'}`} style={{ marginTop: 'var(--space-2)' }}>
                EOD: {eodMsg}
              </div>
            )}
            {pcrMsg != null && (
              <div className={`sdp-feedback sdp-msg--${pcrOk ? 'ok' : 'err'}`} style={{ marginTop: 'var(--space-2)' }}>
                PCR: {pcrMsg}
              </div>
            )}
          ...
          <>
            ...
            {criteriaStats != null && (
              <div className="sdp-eval-body" style={{ marginTop: 'var(--space-4)' }}>
                <div className="sdp-eval-cols">
                {/* ── Left column: Fundamental ─── */}
                <div className="sdp-eval-col">
                {/* Fundamental */}
                ...
                <div className="sdp-eval-section">
                  ...
                  {criteriaStats.technical.momentum_conditions && criteriaStats.technical.momentum_conditions.length > 0 && (
                    <div className="sdp-eval-tier-section">
                      <div className="sdp-eval-tier-head">
                        <span className="sdp-criteria-group-badge sdp-criteria-group-badge--momentum">MOMENTUM</span>
                        <span className="sdp-check-secondary sdp-eval-tier-sub">Scored 0–10</span>
                      </div>
                      <div className="sdp-criteria-rows">
                  ...
                  {criteriaStats.technical.structure_conditions && criteriaStats.technical.structure_conditions.length > 0 && (
                    <div className="sdp-eval-tier-section">
                      <div className="sdp-eval-tier-head">
                        <span className="sdp-criteria-group-badge sdp-criteria-group-badge--structure">STRUCTURE</span>
                        <span className="sdp-check-secondary sdp-eval-tier-sub">Diagnostics</span>
                      </div>
                      <div className="sdp-criteria-rows">
                  ...
                  {criteriaStats.technical.sentiment_conditions && criteriaStats.technical.sentiment_conditions.length > 0 && (
                    <div className="sdp-eval-tier-section">
                      <div className="sdp-eval-tier-head">
                        <span className="sdp-criteria-group-badge sdp-criteria-group-badge--sentiment">SENTIMENT</span>
                        <span className="sdp-check-secondary sdp-eval-tier-sub">Short Interest / Volume</span>
                      </div>
                      <div className="sdp-criteria-rows">
            ...
            <div className="sdp-eval-panel" style={{ marginTop: 'var(--space-4)' }}>
              ...
              <p className="sdp-step-desc sdp-text-dim">
                          ...
                          <td>
                            <div className="sdp-inventory-fill-wrap">
                              <div className="sdp-inventory-fill-bar">
                                <div
                                  className={`sdp-inventory-fill-bar-inner${pct >= 70 ? ' sdp-inventory-fill-bar-inner--ok' : pct >= 40 ? ' sdp-inventory-fill-bar-inner--warn' : ' sdp-inventory-fill-bar-inner--err'}`}
                                  style={{ width: `${pct}%` }}
                                />
                              </div>
