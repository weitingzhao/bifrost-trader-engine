...
export async function fetchPerformance(params?: {
  ...
  /** Omit or performance_book (default) = account_executions_final; on_the_fly = account_executions_fly */
  ...
  const r = await fetch(tradingUrl(`/performance?${search}`))
  if (!r.ok) throw new Error(r.statusText)
  return r.json()
}
...
export async function getTransactions(params?: {
  ...
  const r = await fetch(tradingUrl(`/transactions?${search}`))
  if (!r.ok) throw new Error(r.statusText)
  return r.json()
}
...
export async function postTransactionsFetch(body?: { from_date?: string; to_date?: string }): Promise<{ ok: boolean; count?: number; message?: string; error?: string; by_account?: number }> {
  const r = await fetch(tradingUrl('/transactions/fetch'), {
    method: 'POST',
